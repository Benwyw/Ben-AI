...
class Pres(commands.Cog):
    @commands.command(description="Play cards by the index displayed underneath them.",
                      ...
                      pass_context=True)
    async def enter(self, ctx, *args: int):
        ...
        if not checkInGame(ctx.author):
            ...
            embed.set_footer(text="Use $in <game ID> to join an existing game.")
    ...
    async def enter_error(self, ctx, error):
        if isinstance(error, commands.BadArgument) or isinstance(error, commands.MissingRequiredArgument):
            embed = discord.Embed(title="Command Error", description="Invalid arguments detected for command 'play'. Check $cmd enter for more details.", color=0x00ff00)
            embed.set_author(name=ctx.author.display_name, icon_url=ctx.author.avatar_url)
