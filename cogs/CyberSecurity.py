...
class CyberSecurity(commands.Cog):
    ...
    security = SlashCommandGroup(guild_ids=guild_ids, name="security", description='CyberSecurity',
                                 description_localizations={"zh-TW": "電腦安全"})

    @security.command(guild_ids=guild_ids, name='scan', description='Retrieve virus scan report of the url',
                      cooldown=commands.CooldownMapping.from_cooldown(2, 60, commands.BucketType.default),
                      description_localizations={"zh-TW": "提取網址的病毒掃描報告"})
    # @commands.cooldown(1, 600, commands.BucketType.user)
    async def _scan(self, ctx: commands.Context,
                    url: Option(str, "Paste the url", name_localizations={"zh-TW": "貼上網址"})):
        await ctx.defer()
            ...
            if uri_validator(url_param):
                if '200' in str(post_scan(url_param, VIRUS_TOTAL_API_KEY)):
                    ...
                    if ctx.guild.id in (guild_BenKaChu, guild_BrianLeeGaming, guild_BrianLeeGaming):  # chi
                        chi_embed = discord.Embed(title="防毒Ben件", color=color,
                                                  description=f"使用與{total_vendor_count}位防毒供應商合作的VirusTotal。")
                        chi_embed.set_author(name='VirusTotal',
                                             url=f"https://www.virustotal.com/gui/url/{id}?nocache=1",
                                             icon_url="https://i.imgur.com/E7cIWC1.png")
                        ...
                        eng_embed = discord.Embed(title="AntiBenrus", color=color,
                                                  description=f"An API call to VirusTotal with {total_vendor_count} Security Vendors.")
                        eng_embed.set_author(name='VirusTotal',
                                             url=f"https://www.virustotal.com/gui/url/{id}?nocache=1",
                                             icon_url="https://i.imgur.com/E7cIWC1.png")
